...
# model_list = ['KNN', 'RF', 'SVM', 'AdaBoost', 'ExtraTrees', 'LightGBM']
...
task_list = [
    ...
    {
        'model_list': ["LR", "RF"],
        'dataset_list': ['WADI', 'NIPS_TS_Swan', 'NIPS_TS_Water'],
        'extra_config': {"LR": {'win_size': 1}, "RF": {'win_size': 1}}
    }
]
        ...
        if dataset == 'UCR':
            ...
            for idx in index_range:
                ...
                cmd = ['python', 'supervised.py', '--model_name', 'STAND', '--dataset_name', dataset, '--index', str(idx),
                       '--task_name', 'supervised', '--train_test_split', str(train_test_split), '--win_size', win_size]
                subprocess.run(cmd)
        else:
            ...
            cmd = ['python', 'supervised.py', '--model_name', 'STAND', '--dataset_name', dataset,
                   ...
                   '--train_test_split', str(train_test_split), '--win_size', win_size]
            subprocess.run(cmd)
...
if __name__ == '__main__':
    ...
    for train_test_split in train_test_split_list:
        if run_task == 0:
            run_baselines(model_list, dataset_list, train_test_split)
