        ...
        for dataset in dataset_list:
            if dataset == 'UCR':
                ...
                for idx in index_range:
                    ...
                    cmd = ['python', 'supervised.py', '--model_name', model, '--dataset_name', dataset, '--index', int(idx), 
...
def run_stand(train_test_split=0.5):
    for dataset in dataset_list:
        if dataset == 'UCR':
            index_range = range(1, 251)
            for idx in index_range:
                print(f'Running experiment for model: STAND, dataset: {dataset} with index: {idx}')
                cmd = ['python', 'supervised.py', '--model_name', 'STAND', '--dataset_name', dataset, '--index', int(idx),
                       '--task_name', 'supervised', '--train_test_split', train_test_split]
                subprocess.run(cmd)
        else:
            print(f'Running experiment for model: STAND, dataset: {dataset}')
            cmd = ['python', 'supervised.py', '--model_name', 'STAND', '--dataset_name', dataset, '--task_name', 'supervised',
                   '--train_test_split', train_test_split]
            subprocess.run(cmd)
