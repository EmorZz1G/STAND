...
def run_stand(dataset_list, train_test_split=0.5, win_size=32, num_layers=1, d_model=256, bidirectional=1):
...
if __name__ == '__main__':
    ...
    for train_test_split in train_test_split_list:
        for model in model_list:
            for dataset in dataset_list:
                if model == 'STAND':
                    run_stand([dataset], train_test_split=train_test_split)