...
assert config.task_name == 'unsupervised', 'This script only supports unsupervised task!'
import pathlib
...
logs_file_pth = logs_pth / 'unsupervised_exp.csv'
logs_file_pth3 = logs_pth / 'unsupervised_exp_left_test.csv'
import os
import sys
sys.path.append(str(proj_pth))

from src.utils.model_wrapper import run_Unsupervise_AD, Unsupervise_AD_Pool
...
assert config.model_name in Unsupervise_AD_Pool, f"Model {config.model_name} not in {Unsupervise_AD_Pool}"
config_dict_tmp = config_dict.copy()
config_dict_tmp.pop('model_name')
test_score = run_Unsupervise_AD(config.model_name, test_y, **config_dict_tmp)
