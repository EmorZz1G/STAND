...
supervise_log = logs_pth / 'supervised_limit_exp.csv'
import pandas as pd
...
task_list_generator = product(dataset_list, anomaly_ratio_list, win_size_list, d_model_list, num_layers_list, bidirectional_list)
...
for task in task_list_generator:
    tmp = df[(df['dataset_name']==task[0]) & (df['anomaly_ratio']==task[1]) & (df['win_size']==task[2]) & (df['d_model']==task[3]) & (df['num_layers']==task[4]) & (df['bidirectional']==task[5])]
    if tmp.shape[0]==0:
        cnt+=1
